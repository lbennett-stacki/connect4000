...
#[derive(Debug)]
pub enum View<'a> {
    Snapshot(&'a mut Vec<u8>, Option<u64>, u64, u64),
    Joined(u64, u8),
}
...
impl<'a> View<'a> {
    fn serialize(view: View) -> Vec<u8> {
        match view {
            ...
            View::Snapshot(snapshot, winner_id, col_count, row_count) => {
            ...
            match action {
                ...
                Actions::Join(view_tx) => {
                    ...
                    let player = match next_index {
                        0 => Player::red(player_id),
                        1 => Player::yellow(player_id),
    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            sock_tx
                .write()
                .await
                .write_all(&View::serialize(View::Joined(player_id, color.serialize())))
                .await
                .unwrap();
            ...
            sock_tx
                .write()
                .await
                .write_all(&View::serialize(View::Snapshot(
                    &mut snapshot.0,
                    snapshot.1,
                    snapshot.2,
                    snapshot.3,
                ...
                loop {
                    ...
                    match command {
                        Command::PlayCoin(column) => {
                            ...
                            let serialized = View::serialize(View::Snapshot(
                                &mut snapshot.0,
                                snapshot.1,
                                snapshot.2,
                                snapshot.3,
